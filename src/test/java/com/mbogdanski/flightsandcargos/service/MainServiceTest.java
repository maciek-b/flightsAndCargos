...
public class MainServiceTest {
  ...
  public void should_return_empty_weight_response_when_flight_does_not_exist() {
    when(flightRepository.findByFlightNumberAndDepartureDate(DEPARTING_FLIGHT.getFlightNumber(), DEPARTING_FLIGHT.getDepartureDate().toLocalDate()))
        .thenReturn(emptyList());

    final WeightResponse weightResponse = mainService.calcWeight(DEPARTING_FLIGHT.getFlightNumber(), DEPARTING_FLIGHT.getDepartureDate().toLocalDate());
  ...
  public void should_return_calculated_flight_response() {
    when(flightRepository.findByDepartureAirportAndDepartureDate(AIRPORT, DATE.toLocalDate()))
        .thenReturn(List.of(DEPARTING_FLIGHT));
    when(flightRepository.findByArrivalAirportAndDepartureDate(AIRPORT, DATE.toLocalDate()))
    ...
    final FlightResponse flightResponse = mainService.getFlightsInfo(AIRPORT, DATE.toLocalDate());
